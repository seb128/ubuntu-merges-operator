        ...
        for dist in DISTROS[distro]["dists"]:
            for component in DISTROS[distro]["components"]:
                ...
                for source in get_sources(distro, dist, component):
                    ...
                    this_patch_rss = read_rss(
                        patch_rss_file(distro, source),
                        title="Ubuntu Patches from Debian for %s"
                        % source["Package"],
                        link=(
                    ...
                    this_diff_rss = read_rss(
                        diff_rss_file(distro, source),
                        title="Ubuntu Uploads for %s" % source["Package"],
                        link=(
                            "http://patches.ubuntu.com/by-release/atomic/"
                            + tree.subdir(
                                "%s/diffs" % ROOT,
...
def read_watermark(distro, source):
    ...
    mark_file = "%s/%s/watermark" % (
        ROOT,
        pool_directory(distro, source["Package"]),
    )
