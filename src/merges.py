...
REPO_URL = "https://git.launchpad.net/~seb128/merge-o-matic"
...
class Merges:
    ...
    def install(self):
        ...
        try:
            ...
            logger.warning("Creating directory %s failed: %s", datadir, e)
            raise

        try:
            comments = SRVDIR / "comments.txt"
            gid = grp.getgrnam("www-data").gr_gid
            with open(comments, "w") as _:
                pass
            os.chown(comments, -1, gid)
        except OSError as e:
        ...
        try:
            ...
            logger.debug("Merge-o-matic vcs cloned.")
        except CalledProcessError as e:
