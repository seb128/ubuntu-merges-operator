...
class UbuntuMergesCharm(ops.CharmBase):
    ...
    def _on_install(self, event: ops.EventBase):
        ...
        try:
            ...
            self._merges.setup_systemd_units()
        except (
            CalledProcessError,
            PackageError,
    ...
    def _on_collect_status(self, event: ops.CollectStatusEvent):
        """Report the status of the job (runs after each event)."""
        logger.debug("Status collect called")

        if systemd.service_running("ubuntu-merges.service"):
            event.add_status(ops.ActiveStatus("Updating..."))
        elif systemd.service_failed("ubuntu-merges.service"):
            event.add_status(ops.ActiveStatus("Failed to refresh the report."))
        else:
